...
/* IPHONE-BINUTILS LOCAL begin arm struct hack */
/* Cross-reference with arm_struct_value_rtx: there actually *is* a hidden
 * first argument in this case on ARM Mach-O, even though this function lies
 * and returns 0. This is considered a bug in LLVM, and has been registered
 * as such. */
#if 0
!targetm.calls.struct_value_rtx (0, 0) &&
#endif
/* IPHONE-BINUTILS LOCAL end arm struct hack */
        (TREE_CODE (ret_type) == RECORD_TYPE
	      || TREE_CODE (ret_type) == UNION_TYPE)
	  /* APPLE LOCAL begin radar 5080710 */
	  && (TREE_ADDRESSABLE (ret_type) 
