...
arm_init_builtins (void)
{
  if (TARGET_REALLY_IWMMXT)
    ...
    /* IPHONE-BINUTILS LOCAL begin constant cfstrings */
    SUBTARGET_INIT_BUILTINS;
    /* IPHONE-BINUTILS LOCAL end constant cfstrings */
}
...
{
  /* Do nothing. We need this symbol, but we don't need
   * to do anything here when we're backed by LLVM */
}

/* APPLE LOCAL begin optimization pragmas 3124235/3420242 */
/* Version of the above for use from #pragma optimization_level.
   Do not reset things unless they're per-function.  */
void reset_optimization_options (int level ATTRIBUTE_UNUSED, int size)
{
  /* XXX: What optimizaton flags should we reset here? Should be mostly safe to do nothing? */
}
